 ...
 */
export type CaseType = "lowercase" | "uppercase" | "sentence" | "title" | "snake" | "kebab" | "camel" | "pascal" | "dot" | "constant";
...
export function extractWords(input: string): string[] {
  ...
  const regex = /[^a-zA-Z0-9]+|(?<=[a-z])(?=[A-Z])|(?<=[A-Z])(?=[A-Z][a-z])|(?<=[a-zA-Z])(?=[0-9])|(?<=[0-9])(?=[a-zA-Z])/;
...
export function convertCase(input: string, caseType: CaseType): string {
  ...
  switch (caseType) {
    ...
    case "camel":
      return words.length === 1 ? words[0]!.toLowerCase() : words[0]!.toLowerCase() + words.slice(1).map(capitalize).join("");
